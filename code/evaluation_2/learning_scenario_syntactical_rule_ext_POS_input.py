...
###################  DO NOT UNCOMMENT THIS df1.to_csv("/home/na/PycharmProjects/log_level_estimation/log_level_estimation/3_preprocessed_data/learning_scenario_3/top100_repositories.csv")
...
for seed in range(10):
    df = pd.read_csv("../../3_preprocessed_data/lingustic_properties_data/lingustic_quality_inter.csv")
    df = df.loc[:, ["pos", "label"]]
    df.columns = ["log_messages", "label"]
    ...
    class_count = df.groupby("label").count().reset_index()['label']
    ...
    for train_index, test_index in stSampling.split(load_train1, load_train_labels1):
        ...
        weights = np.array([0.9, 1]) # weights = np.array([2.5, 1])
        ...
        # torch.save(model,"/home/na/PycharmProjects/huawei_qulog/log_level_estimation/5_results/sytactic_evaluation/lignustic_evaluation_model.pth")
        #
        # with open(
        #         "/home/na/PycharmProjects/huawei_qulog/log_level_estimation/5_results/sytactic_evaluation/tokenizer.pickle",
        #         "wb") as file:
